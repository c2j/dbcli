...
fn format_value_by_type(row: &Row, idx: usize, col_type: ColumnType) -> Value {
    match col_type {
        ...
        ColumnType::MYSQL_TYPE_TINY => {
            get_val::<i8>(row, idx).map_or(Value::Null, |v| json!(v))
        }
        ...
        ColumnType::MYSQL_TYPE_FLOAT => {
            get_val::<f32>(row, idx).map_or(Value::Null, |v| json!(v))
        }

        ColumnType::MYSQL_TYPE_DOUBLE => {
            get_val::<f64>(row, idx).map_or(Value::Null, |v| json!(v))
        }
        ...
        ColumnType::MYSQL_TYPE_BIT => {
            get_val::<bool>(row, idx).map_or(Value::Null, |v| json!(v))
        }
